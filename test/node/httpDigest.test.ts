...
import { createHeaderValue, verifyHeaderValue } from '../../src/index.js'
...
describe('http-signature-digest', () => {
  describe('createDigestString', () => {
    ...
    it('should create a digest of a given string with useMultihash set to true', async () => {
      const data = `{"hello": "world"}`
      let result
      let err
      try {
      ...
      expect(err).toBeUndefined()
      expect(result).toBeDefined()
      expect(result).toBe('mh=uEiBfjwT2o6iSqqu922zyc4lEk3c5YNSjJbEF_uRu70ME8Q')
    })
    ...
    it('should create a digest of an object', async () => {
      ...
      expect(err).toBeUndefined()
      expect(stringDigest).toBeDefined()
      expect(objDigest).toBe(stringDigest)
      expect(objDigest).toBe(
    ...
    it('should create a digest of an object with useMultihash set to true', async () => {
      const data = { hello: 'world' }
      ...
      expect(err).toBeUndefined()
      expect(stringHeaderValue).toBeDefined()
      expect(headerValue).toBe(stringHeaderValue)
  ...
  describe('verifyDigestString', () => {
    ...
    it('should verify a digest wrapped in colons', async () => {
      const data = `{"hello": "world"}`
      const headerValue = `sha-256=:X48E9qOokqqrvdts8nOJRJN3OWDUoyWxBf7kbu9DBPE=:`
      let verifyResult
      let err
      try {
        verifyResult = await verifyHeaderValue({ data, headerValue })
    ...
    it('should verify false if a multihash digest is wrapped in colons', async () => {
      const data = '{"hello":"world"}'
      const headerValue = `mh=:uEiCTojlxqRTl6svwqNJRVM2jCcPBxy-7mRTUfGDzy2gViA:`
      let verifyResult
      let err
      try {
        verifyResult = await verifyHeaderValue({ data, headerValue })
    ...
    it('should verify false if hashedDigestStringValue and headerValue are not equal when header is multihash', async () => {
      const data = `{"hello": "world"}`
      const headerValue = await createHeaderValue({ data, useMultihash: true })
      const dataToVerify = `{"hello": "earth"}`
      let verifyResult
    ...
    it('should verify true if hashedDigestStringValue and headerValue are equal when header is multihash', async () => {
      const data = `{"hello": "world"}`
      const headerValue = await createHeaderValue({ data, useMultihash: true })
      let verifyResult
      let err
